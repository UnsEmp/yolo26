...
def train():
    ...
    model = YOLO("datasets/Cow-eat-chew-behaviour/yolo26n_8.yaml")  # Load a model from a YAML configuration file
    # model = YOLO("runs/detect/Cow-EatOrChew-train-yolo26n-C3k2Star/weights/best.pt")

    # Train the model  for  epochs
    train_results = model.train(
        data="datasets/Cattle_Behavior721_coco/Cattle_Behavior721_coco.yaml",  # Path to dataset configuration file
        epochs=300,  # Number of training epochs
        imgsz=640,  # Image size for training
        device="0",  # Device to run on (e.g., 'cpu', 0, [0,1,2,3])

        mpdiou=True,  # Use MPDIoU loss for bounding box regression
        resume=True,
        workers=4,
        patience=100,  # Early stopping patience (number of epochs with no improvement after which training will be stopped)
        batch=16,
        name="Cow-EatOrChew-train-yolo26n-Cattle_Behavior721_coco-SPDSGhost-C3k2Star-CBAM-4EMA-BiFPN",  # Name for the training run, used for saving results
    ...
    # calcula123
    # te average FPS after training
    fps_list = [[], [], [], []]
    for ep in range(10):
        metrics = model.val(project='runs/detect/Cow-EatOrChew-train-yolo26n-MSAC2PSA',
                            data="datasets/Cattle_Behavior721_coco/Cattle_Behavior721_coco.yaml",
                            imgsz=640,
                            batch=1,
                            device="0"
                            )
