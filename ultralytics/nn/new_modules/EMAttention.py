...
class EMA(nn.Module):
    """EMA++: Export-friendly + stronger for small objects & occlusion Key upgrades: 1) Replace AdaptivePool(None,1)
    with reduce ops (mean/max) for export stability 2) Add channel gate (ECA/SE-like) to complement
    coordinate/spatial weighting 3) Residual scaling gamma (init 0) to prevent over-suppression, improve recall 4)
    Use DWConv large-kernel for local context at low cost.
    """
    ...
    def __init__(self, channels, factor=32, reduction=16, k_large=5, tau=1.0):
        ...
        self.conv1x1 = nn.Sequential(nn.Conv2d(c_g, c_g, kernel_size=1, bias=False), nn.BatchNorm2d(c_g), nn.SiLU())
        ...
        self.pw = nn.Sequential(nn.Conv2d(c_g, c_g, kernel_size=1, bias=False), nn.BatchNorm2d(c_g), nn.SiLU())
