...
#         # Apply the 1D convolution to the pooled tensor
#         y = self.conv(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)

#         # Ensure the channel dimension of y matches x before applying attention
#         if y.shape[1] != x.shape[1]:
#             # Ensure y's channel dimension matches x's channel dimension
#             y = y.expand_as(x)  # Expand y to match x's shape

#         # Apply sigmoid activation
#         y = self.sigmoid(y)

#         return x * y  # Apply the attention mechanism to the input tensor
...
class CBAM(nn.Module):
    ...
    def __init__(self, c1, kernel_size=7):
        ...
        # print("c1, kernel_size:", c1, kernel_size)
        super().__init__()
