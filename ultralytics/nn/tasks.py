...
def parse_model(d, ch, verbose=True):
    ...
    # 将ch变量的值包装成一个列表
    ch = [ch]
    # layers是将要添加到模型的层 save是中间层的索引列表 c2是输出通道
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # 下面代码的主要目的是根据 m 的值调整args列表。根据不同的模块类型提取重新排列或添加参数以满足特定模块的需求。
            ...
            if m is C2fCIB:
                legacy = False
            if m is CBAM:
                # print("ch, args : ", ch, args)
                args = [ch[f]]  # set input channels to the previous layer's output channels
            if m is GAM:
                args = [ch[f]]
            if m is ECA:
                args = [ch[f]]
