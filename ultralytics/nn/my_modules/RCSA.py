...
class RCSA(nn.Module):
    """
    肉牛进食识别专用注意力机制：Ruminant Channel-Spatial Attention (RCSA)
    输入: 特征图 (b, c, h, w)
    输出: 加权特征图 (b, c, h, w)
    """
    ...
    def __init__(self, channels, factor=32):
        ...
        self.channel_conv = nn.Conv2d(2 * (channels // self.groups), channels // self.groups, 
        ...
        self.spatial_conv1x1 = nn.Conv2d(channels // self.groups, channels // self.groups, 
                                           kernel_size=1, stride=1, padding=0)
        self.spatial_conv3x3 = nn.Conv2d(channels // self.groups, channels // self.groups, 
                                           kernel_size=3, stride=1, padding=1)
        self.spatial_gn = nn.GroupNorm(channels // self.groups, channels // self.groups)
        self.spatial_sigmoid = nn.Sigmoid()

        # 位置编码（Coordinate Attention简化版）
        self.pos_conv_h = nn.Conv2d(channels // self.groups, channels // self.groups, 
                                     kernel_size=1, stride=1, padding=0)
        self.pos_conv_w = nn.Conv2d(channels // self.groups, channels // self.groups, 
                                     kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        channel_weight = self.channel_sigmoid(
            self.channel_gn(self.channel_conv(channel_cat))
        )  # (b*g, c//g, 1, 1)
        ...
        spatial_weight = self.spatial_sigmoid(
            self.spatial_gn(spatial_x)
        )  # (b*g, c//g, h, w)
...
class C2f_RCSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            Bottleneck_RCSA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) 
            for _ in range(n)
        )
