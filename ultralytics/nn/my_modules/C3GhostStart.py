...
class GhostStarBlock(nn.Module):
    """
    GhostStarBlock: 
    结合 Ghost 的轻量化 (DWConv) 和 StarNet 的高维映射 (Element-wise Mul)
    结构: Conv(1x1) -> DWConv(3x3) -> Split -> Mul -> Conv(1x1)
    """
...
class C3GhostStar(nn.Module):
    """
    CSP结构封装 GhostStarBlock
    比 C2f 轻量，比 C3Ghost 精度高
    """
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            GhostStarBlock(self.c, self.c) for _ in range(n)
        )
