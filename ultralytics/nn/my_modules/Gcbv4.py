...
class Gcbv4(nn.Module):
    """
    GCBv4 (Ghost Conv Branch v4)
    结构逻辑:
    1. Input (c1) -> 分为两个主分支
    2. Branch 1: GhostConv (c1 -> c1)
    3. Branch 2:
       - Split: 切分为两半 (c1/2, c1/2)
       - Sub-branch 2a: MaxPool (保留纹理/边界)
       - Sub-branch 2b: AvgPool (保留背景) * Learnable Parameter
       - Combine: Element-wise Add (2a + 2b)
       - BatchNorm
    4. Concat: Branch 1 (c1) + Branch 2 (c1/2) = 1.5 * c1 通道
    5. Final: GhostConv (1.5*c1 -> c2) 调整回目标大小
    """
