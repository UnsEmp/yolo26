...
class Gcbv3(nn.Module):
    """Gcbv3 (Ghost Conv Branch v3) Module 结构描述: 1. Input -> 分为三个支路 2. Branch 1: GhostConv (保持通道不变) -> MaxPool
    (stride=1, 保持尺寸) 3. Branch 2: AvgPool (stride=1, 保持尺寸) 4. Branch 3: Identity (原样保持) 5. Fusion: Element-wise Add
    (Branch1 + Branch2 + Branch3) 6. Output: GhostConv (调整通道到 c2).
    """
